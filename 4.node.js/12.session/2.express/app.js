...
app.use(
  session({
    ...
    resave:false, //세션 데이터가 변경되지 않았어도 계속 재저장할거냐
    saveUninitialized:trues, //초기화되지 않은 세션을 저장소에 저장할거냐
  }));

function userCount(req,res,next){
    //세션에 방무카운트 변수 있으면 재사용, 없으면 0으로 초기화
    req.session.visitCount=req.session.visitCount||0;

    req.session.visitCount++;

    console.log(`이 SessionID: ${req.sessionID}는 몇번째 방문임:`
        next();
...
app.get("/readsession", (req, res) => {
  ...
  if (username && cart) {
    res.send(`너느 ${username}이고, ${visitCount}번째 방문했고, 이전에 장바구니에 ${cart}이걸 담았어.`);
  } else {
